...
<script type="text/javascript">
     ...
     */
    function connect() {
        stompClient = Stomp.client("ws://172.30.22.10:9000/acceptorEndpoint", 'v11.stomp');
...
//        headers.id="sub_"+userId;//自定义订阅的id
        //订阅审核者频道
        stompClient.subscribe('/user/' + userId + '/acceptorChannel', onMessage,headers);
    };
    /**
     * 连接出现异常后的回调函数
     */
...
//        frame.nack();//通知服务端消息处理失败
        ...
        var dtoJson=JSON.parse(frame.body);
        if(dtoJson){
            var type=dtoJson.type;
            if(type=="close"){
                this.disconnect();
            }else if(type=="video"){
                var resId=dtoJson.resourceId;
                var videoUrl=dtoJson.videoUrl; //根据url打开视频
                var intopieceNo=dtoJson.intopieceNo;
                //弹窗
            }
     ...
     * 取消视频审核
     */
    function refuseAccept(resId) {
        //第一个参数是STOMP destination（必选），第二个参数是headers（可选），第三个参数是body（可选）
        stompClient.send("/app/accept/refuseAccept", {}, JSON.stringify({'resourceId': resId}));
    }
    /**
     * 断开连接
     */
    ...
    function sendMsg() {
        var name = $('#name').val();
        //第一个参数是STOMP destination（必选），第二个参数是headers（可选），第三个参数是body（可选）
        stompClient.send("/app/accept/refuseAccept", {}, JSON.stringify({'name': name, 'userId': userId}));
    }
