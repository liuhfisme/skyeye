...
<div>
    ...
    <div id="conversationDiv">
        <button id="sendName" onclick="sendMsg();">发起视频</button>
        <p id="response"></p>
    </div>
</div>
<script type="text/javascript">
     ...
     */
    function connect() {
        stompClient = Stomp.client("ws://127.0.0.1:9000/applicantEndpoint", 'v11.stomp');
...
//        headers.id="sub_"+userId;//自定义订阅的id
        //订阅申请者频道
        stompClient.subscribe('/user/' + userId + '/applicantChannel', onMessage,headers);
    };
    /**
     * 连接出现异常后的回调函数
     */
...
//        frame.nack();//通知服务端消息处理失败
     ...
     */
    function sendMsg() {
        var applyInfoJson=JSON.parse('${applyInfo}');
        applyInfoJson.userId=userId;
        alert(JSON.stringify(applyInfoJson))
        //第一个参数是STOMP destination（必选），第二个参数是headers（可选），第三个参数是body（可选）
        stompClient.send("/app/applyVideo", {}, JSON.stringify(applyInfoJson));
    }
